...
class LicenseBoxExternalAPI{
	...
	public function __construct(){ 
		$this->product_id = 'A8EEC348';
		$this->api_url = 'http://license.beepbag.com/';
		$this->api_key = '3A919AA46AE5D02FA7F6';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["31a5eefbc1ea8d2"])){
				$_SESSION["31a5eefbc1ea8d2"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["31a5eefbc1ea8d2"])){
				$get_data = $this->call_api(
					'POST',
